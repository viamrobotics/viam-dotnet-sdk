...
{
    public class StructMapperGenerator_Old : IIncrementalGenerator
    {
                ...
                if (!AreAllTypesPartial(classSymbol))
                {
                    context.ReportDiagnostic(Diagnostic.Create(
                        new DiagnosticDescriptor(
                            ...
                            messageFormat:
                            "Class '{0}' and all containing classes must be marked 'partial' to allow source generation.",
        ...
        private static string GenerateStructMapper(INamedTypeSymbol classSymbol, SourceProductionContext context)
        {
            ...
            sb.AppendLine($"{indent}{{");
            sb.AppendLine(
                $"{indent}    public static {fullyQualifiedName} FromStruct(Google.Protobuf.WellKnownTypes.Struct s)");
            sb.AppendLine($"{indent}    {{");
            ...
            foreach (var prop in classSymbol.GetMembers().OfType<IPropertySymbol>())
            {
                ...
                Trace(context,
                    $"Processing property '{propName}' of type '{prop.Type}' with original type: '{prop.Type.OriginalDefinition.ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat)}'",
                ...
                else if (prop.Type is INamedTypeSymbol namedType)
                {
                    ...
                    if (IsDictionaryish(namedType))
                    {
                        ...
                        {
                            Trace(context,
                                $"Skipping Dictionary<{namedType.TypeArguments[0]}, {namedType.TypeArguments[1]}> — only Dictionary<string, T> is supported");
                            sb.AppendLine(
                                $"{indent}        // Skipping Dictionary<{namedType.TypeArguments[0]}, {namedType.TypeArguments[1]}>: unsupported key type");
                            continue;
                        }
                        ...
                        {
                            Trace(context,
                                $"Skipping Dictionary<{keyType}, {valueType}> — only Dictionary<string, T> is supported");
                            sb.AppendLine(
                                $"{indent}        // Skipping Dictionary<{keyType}, {valueType}>: unsupported key type");
                        }
                    }
                    else if (IsListIsh(namedType))
                    {
                        Trace(context,
                            propName + " is an enumerable: " +
                    ...
                    else
                    {
                        ...
                        sb.AppendLine(
                            $"{indent}if (s.Fields.TryGetValue(\"{fieldName}\", out var {propName}_val) && {propName}_val.KindCase == Google.Protobuf.WellKnownTypes.Value.KindOneofCase.StructValue)");
                        sb.AppendLine($"{indent}{{");
                        sb.AppendLine(
                            $"{indent}    result.{propName} = {prop.Type.Name}.FromStruct({propName}_val.StructValue);");
                        sb.AppendLine($"{indent}}}");
                    }
                }
        ...
        private static void HandleDictionary(StringBuilder sb, string indent, SourceProductionContext context,
            IPropertySymbol prop, ITypeSymbol valueType)
        {
            var propName = prop.Name;
            ...
            indent += "    ";
            sb.AppendLine(
                $"{indent}result.{propName} = new global::System.Collections.Generic.Dictionary<string, {valueTypeName}>();");
            ...
            else if (valueType.SpecialType == SpecialType.System_Single ||
                     valueType.SpecialType == SpecialType.System_Double)
            {
            ...
            else
            {
                ...
                sb.AppendLine(
                    $"{indent}if (kvp.Value.KindCase == Google.Protobuf.WellKnownTypes.Value.KindOneofCase.StructValue)");
                sb.AppendLine($"{indent}{{");
                sb.AppendLine(
                    $"{indent}    result.{propName}[kvp.Key] = {valueTypeName}.FromStruct(kvp.Value.StructValue);");
                sb.AppendLine($"{indent}}}");
        ...
        private static void HandleSpecialType(StringBuilder sb, string indent, SourceProductionContext context,
            IPropertySymbol prop)
        {
            ...
            switch (prop.Type.SpecialType)
            {
                case SpecialType.System_String:
                    sb.AppendLine(
                        $"{indent}result.{propName} = s.Fields.TryGetValue(\"{fieldName}\", out var {propName}_val) ? {propName}_val.StringValue : \"\";");
                    break;
                case SpecialType.System_Int16:
                case SpecialType.System_Int32:
                case SpecialType.System_Int64:
                    sb.AppendLine(
                        $"{indent}result.{propName} = s.Fields.TryGetValue(\"{fieldName}\", out var {propName}_val) ? (int){propName}_val.NumberValue : 0;");
                    break;
                case SpecialType.System_Boolean:
                    sb.AppendLine(
                        $"{indent}result.{propName} = s.Fields.TryGetValue(\"{fieldName}\", out var {propName}_val) ? {propName}_val.BoolValue : false;");
                    break;
                case SpecialType.System_Single:
                case SpecialType.System_Double:
                    sb.AppendLine(
                        $"{indent}result.{propName} = s.Fields.TryGetValue(\"{fieldName}\", out var {propName}_val) ? {propName}_val.NumberValue : 0;");
                    break;
                case SpecialType.System_Decimal:
                    sb.AppendLine(
                        $"{indent}result.{propName} = s.Fields.TryGetValue(\"{fieldName}\", out var {propName}_val) ? (decimal){propName}_val.NumberValue : 0;");
                    break;
                default:
                    sb.AppendLine($"{indent}// Unknown type '{prop.Type}' for property '{propName}'");
                    Trace(context,
                        "Unknown special type: " + prop.Type.ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat));
                    break;
            }
        }

        private static void HandleList(StringBuilder sb, string indent, SourceProductionContext context,
            IPropertySymbol prop, ITypeSymbol elementType)
        {
            var propName = prop.Name;
            var fieldName = GetJsonPropertyName(prop);
            ...
            sb.AppendLine(
                $"{indent}if (s.Fields.TryGetValue(\"{fieldName}\", out var {propName}_val) && {propName}_val.KindCase == Google.Protobuf.WellKnownTypes.Value.KindOneofCase.ListValue)");
            sb.AppendLine($"{indent}{{");
            indent += "    ";
            sb.AppendLine(
                $"{indent}result.{propName} = new global::System.Collections.Generic.List<{elementTypeName}>();");
            ...
            else
            {
                ...
                sb.AppendLine(
                    $"{indent}result.{propName}.Add({elementType.ToDisplayString()}.FromStruct(item.StructValue));");
            }
        ...
        private static void HandleArray(StringBuilder sb, string indent, SourceProductionContext context,
            IPropertySymbol prop, ITypeSymbol elementType)
        {
            var propName = prop.Name;
            var elementTypeName = elementType.ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat);
            ...
            indent += "    ";
            sb.AppendLine(
                $"{indent}var {propName}_arr = new global::System.Collections.Generic.List<{elementTypeName}>();");
            sb.AppendLine($"{indent}foreach (var item in {propName}_val.ListValue.Values)");
            sb.AppendLine($"{indent}{{");
            indent += "    ";
            ...
            else
            {
                Trace(context, $"Assuming nested POCO for property '{propName}' of type '{elementType}'");
                sb.AppendLine(
                    $"{indent}{propName}_arry.Add({elementType.ToDisplayString()}.FromStruct(item.StructValue));");
            }
        ...
        private static string GetJsonPropertyName(IPropertySymbol prop)
        {
            foreach (var attribute in prop.GetAttributes())
            {
                ...
                {
                    if (attribute.ConstructorArguments[0].Value is string jsonName &&
