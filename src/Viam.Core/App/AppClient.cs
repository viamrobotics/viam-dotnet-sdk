    ...
    public sealed class AppClient(AppService.AppServiceClient client)
    {
            ...
            CancellationToken cancellationToken = default)
        {
            await client.CreateOrganizationInviteAsync(new CreateOrganizationInviteRequest()
                    {
                        OrganizationId = await GetOrganizationId(cancellationToken)
                            .ConfigureAwait(false),
                        Email = email,
        ...
        public async ValueTask<(string Id, string Key)> CreateKeyFromExistingKeyAuthorizations(string id,
            CancellationToken cancellationToken = default)
        {
            var result = await client
                .CreateKeyFromExistingKeyAuthorizationsAsync(
                    new CreateKeyFromExistingKeyAuthorizationsRequest() { Id = id },
