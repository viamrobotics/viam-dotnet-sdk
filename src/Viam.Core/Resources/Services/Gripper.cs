...
namespace Viam.Core.Resources.Services
{
    internal class Gripper : GripperService.GripperServiceBase, IServiceBase
    {
        ...
        {
            var resource = (IGripper)context.UserState["resource"];
            await resource.Grab(request.Extra, context.Deadline.ToTimeout(), context.CancellationToken);
            return new GrabResponse();
        }
        public override async Task<OpenResponse> Open(OpenRequest request, ServerCallContext context)
        {
            var resource = (IGripper)context.UserState["resource"];
            await resource.Open(request.Extra, context.Deadline.ToTimeout(), context.CancellationToken);
