...
{
    internal class Arm : ArmService.ArmServiceBase, IServiceBase
    {
        ...
        public override async Task<GetEndPositionResponse> GetEndPosition(GetEndPositionRequest request, ServerCallContext context)
        {
            ...
            var pose = await resource.GetEndPosition(request.Extra, context.Deadline - DateTime.UtcNow, context.CancellationToken);
        ...
        {
            var resource = (IArm)context.UserState["resource"];
            var geometries = await resource.GetGeometries(request.Extra, context.Deadline - DateTime.UtcNow, context.CancellationToken);
        ...
        {
            var resource = (IArm)context.UserState["resource"];
            var jointPositions = await resource.GetJointPositions(request.Extra, context.Deadline - DateTime.UtcNow, context.CancellationToken);
        ...
        {
            var resource = (IArm)context.UserState["resource"];
            var (format, data) = await resource.GetKinematics(request.Extra, context.Deadline - DateTime.UtcNow, context.CancellationToken);
        ...
        {
            var resource = (IArm)context.UserState["resource"];
            await resource.MoveToJoinPositions(request.Positions, request.Extra, context.Deadline - DateTime.UtcNow, context.CancellationToken);
        ...
        {
            var resource = (IArm)context.UserState["resource"];
            await resource.MoveToPosition(request.To, request.Extra, context.Deadline - DateTime.UtcNow, context.CancellationToken);
        ...
        {
            var resource = (IArm)context.UserState["resource"];
            await resource.Stop(request.Extra, context.Deadline - DateTime.UtcNow, context.CancellationToken);
